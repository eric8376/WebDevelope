...
public class JsonHelper {
     ...
     * @param dataFormat 时间日期格式化,传null则表明List不包含日期时间属性
     */
    public static final String encodeList2PageJson(List list, Integer totalCount, String dataFormat, String TOTALCOUNTFlag, String ROOTFlag) {
        String subJsonString = "";
        if (G4Utils.isEmpty(dataFormat)) {
            subJsonString = encodeObject2Json(list);
        } else {
        ...
        String jsonString = "{" + TOTALCOUNTFlag + ":" + totalCount + ",success:true, " + ROOTFlag + ":" + subJsonString + "}";
        if (log.isInfoEnabled()) {
            log.info("序列化后的JSON资料输出:\n" + jsonString);
        }
        return jsonString;
    ...
    /**
     * 提取dto，从web传过来encode过的字符串，重新解析成dto
     *
     * @param string
     * @return
     */
    public static List<Dto> parseJson2DtoFromWeb(String string) {
        if (string.equals("") || string.isEmpty() || string.length() == 0 || string == null || string.equals("[]")) {
            return null;
        }
        List<Dto> costs = new ArrayList<Dto>();
        if (string != null) {
            string = string.substring(1, string.length() - 1);
            String Othercosts[] = string.split("},");
            for (String cost : Othercosts) {
                if (cost.indexOf("}") == -1) {
                    cost = cost + "}";
                }
                costs.add(JsonHelper.parseSingleJson2Dto(cost));
            }
        }
        return costs;
    }
     ...
     * @return
     */
    public static Dto parsejsonComplexJson2Dto(String jsonString) {
        Dto dto = new BaseDto();
        if (G4Utils.isEmpty(jsonString)) {
            return dto;
        }
        if (jsonString.indexOf("[") == 0) {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                list.add(parsejsonComplexJson2Dto(jsonArray.get(i).toString()));
            }
            dto.put("list", list);
        } else {
            ...
            while (iterator.hasNext()) {
                ...
                Dto valueDto = new BaseDto();
                Iterator iteratorSamll = jsonObject.getJSONObject(id).keySet().iterator();
                while (iteratorSamll.hasNext()) {
                    String smallId = (String) iteratorSamll.next();
                    if (!jsonObject.getJSONObject(id).get(smallId).getClass().isArray()) {
                        valueDto.put(smallId, jsonObject.getJSONObject(id).get(smallId));
                    } else {
                        valueDto.put(smallId, parsejsonComplexJson2Dto(jsonObject.getJSONObject(id).getJSONObject(smallId).toString()));
                    }
                }
                dto.put(id, valueDto);
            }
     ...
     * @return
     */
    public static Object parseJson2Object(String json, Class cls) {
        JSONObject jsObj = JSONObject.fromObject(json);
        return JSONObject.toBean(jsObj, cls);
    }
}
