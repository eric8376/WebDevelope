...
//    var miniUpload = new CM.upLoadGrid({
//        fieldLabel: '上传附件', anchor: '100%',
//        showInHeader: false,
//        file_folder : '测试模块',
//        gridSettings: {
...
//        }});
//
//    var minidown = new CM.simpleUpLoadGrid({
//        fieldLabel: '下载附件', anchor: '100%',
//        showInHeader: false,
//        gridSettings: {
//            fileKey: 'T_WF_appendix_file',//文件存储位置(必配)
//            file_folder : '测试模块',
...
//        }});
    ...
    var menuTree =
        Ext.create('Ext.tree.Panel', {
            store: new Ext.data.TreeStore({
                ...
                root: root,
                proxy: {
                    type: 'ajax', url: './CMAppendixAction.ered?reqCode=folderTreeInit'
                }
            }),
            rootVisible: true,
            autoScroll: false,
    ...
    var columns = [
        ...
        {
            header: '文件状态',
            dataIndex: 'delete_state',
            autoWidth: true,
            renderer: function (value) {
                if (value == 0) return '可用';
                else if (value == 1 || value == 2) return '删除';
            }
        }
    ];


    Ext.define('Filed.MenuItems_Model', {
    ...
    var store = Ext.create('Ext.data.Store', {
        model: 'Filed.MenuItems_Model',
        pageSize: '20',
        proxy: {
            extraParams: {},
            type: 'ajax',
            url: './CMAppendixAction.ered?reqCode=queryMenuItemsForManage',
            reader: {
                type: 'json',
...
//                        editInit();
            ...
            '-',
            {
                ...
                handler: function () {
//                        deleteMenuItems('1');
                    Ext.MessageBox.alert('友情提示', '此功能暂时不开发,敬请期待')
                }
            },
            '->',
    ...
    comboxWithTree =
        Ext.create('Ext.ux.TreePicker', {
            ...
            store: new Ext.data.TreeStore({
                ...
                autoLoad: true,
                proxy: {
                    type: 'ajax', url: './CMAppendixAction.ered?reqCode=queryMenuItems'
                }
            }), listeners: {
...
//                },
    ...
    var viewport = new Ext.Viewport({
        ...
        items: [
            {
                title: '<span class="commoncss">文件夹列表</span>',
    ...
    function saveMenuItem() {
        ...
        addMenuFormPanel.form.submit({
            url: './CMAppendixAction.ered?reqCode=saveMenuItem',
            waitTitle: '提示',
            method: 'POST',
            waitMsg: '正在处理数据,请稍候...',
    ...
    function downloadFile() {
        ...
        if (Ext.isEmpty(selectNode)) {
            Ext.MessageBox.alert('提示', '亲,请选择要下载的文件');
            return;
        }
        if (selectNode.length > 1) {
            Ext.MessageBox.alert('提示', '亲,每次只能下一个文件哦');
            return;
        }
        var downloadIframe = document.createElement('iframe');
        downloadIframe.src = 'CMAppendixAction.ered?reqCode=downAppendix&fileId=' + selectNode[0].data.filedid;
    ...
    function deleteMenuItems(pType) {
        ...
        Ext.Msg.confirm('请确认', '你真的要删除选中菜单及其包含的所有子菜单吗?', function (btn, text) {
            if (btn == 'yes') {
                ...
                showWaitMsg();
                Ext.Ajax.request({
                    url: './CMAppendixAction.ered?reqCode=deleteMenuItems',
    ...
    function addInitFolder() {
        ...
        var file_folder = selectNode[0].getId();
        ...
        var addFolderWindow = new Ext.window.Window({
            layout: 'fit',
            width: 420,
            height: 285,
            resizable: false,
            draggable: true,
            closeAction: 'hide',
            title: '<span class="commoncss">附件上传</span>',
            // iconCls : 'page_addIcon',
            modal: true,
            collapsible: true,
            titleCollapse: true,
...
//        Ext.getCmp('windowmode').setValue('add');
    ...
    function update() {
        ...
        var parentid_old = Ext.getCmp('parentid_old').getValue();
        addMenuFormPanel.form.submit({
            url: './CMAppendixAction.ered?reqCode=updateMenuItem',
            waitTitle: '提示',
            method: 'POST',
            waitMsg: '正在处理数据,请稍候...',
            success: function (form, action) {
