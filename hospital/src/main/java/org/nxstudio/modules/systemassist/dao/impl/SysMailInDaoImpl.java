package org.nxstudio.modules.systemassist.dao.impl;

import org.nxstudio.core.dao.impl.GeneralDaoImpl;
import org.nxstudio.core.model.Dto;
import org.nxstudio.modules.systemassist.dao.SysMailInDao;
import org.nxstudio.util.idgenerator.IDHelper;
import org.springframework.stereotype.Repository;

/**
 * Copyright 2000-2014 by RenWoYou Corporation.
 * <p/>
 * All rights reserved.
 * <p/>
 * This software is the confidential and proprietary information of
 * RenWoYou Corporation ("Confidential Information").  You
 * shall not disclose such Confidential Information and shall use
 * it only in accordance with the terms of the license agreement
 * you entered into with RenWoYou.
 * <p/>
 * <p/>
 * Created by Chunji.Luo on 2014/7/30.
 */
@Repository
public class SysMailInDaoImpl extends GeneralDaoImpl implements SysMailInDao {

    public String saveMessageInfo(Dto dto) {
        String codeID = IDHelper.getIdSequenceByKey("SYSEQUENCES");
        dto.put("record_id", codeID);
        this.insert("SystemManage.insertMessageInfo", dto);
        return codeID;
        //COM00102
    }

    public void updateMessageStatu(Dto dto) {
        this.update("SystemManage.updateMessageInfo", dto);
    }
}
