/**
 * @class Ext4.Com.Other.calendar.src.view.AbstractCalendar
 ...
 */
Ext.define('Ext4.Com.Other.calendar.src.view.AbstractCalendar', {
     ...
     * {@link Ext4.Com.Other.calendar.src.EventRecord}) to populate the calendar views with events. Internally this method
    ...
    initComponent: function () {
        ...
        this.addEvents({
             ...
             * @param {Ext4.Com.Other.calendar.src.view.AbstractCalendar} this
             */
            eventsrendered: true,
            /**
             * @event eventclick
             ...
             * @param {Ext4.Com.Other.calendar.src.view.AbstractCalendar} this
             * @param {Ext4.Com.Other.calendar.src.EventRecord} rec The {@link Ext4.Com.Other.calendar.src.EventRecord record} for the event that was deleted
             */
            //eventdelete: true
        });
    },
    ...
    getWeekCount: function () {
        var days = Ext4.Com.Other.calendar.src.util.Date.diffDays(this.viewStart, this.viewEnd);
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function (rec) {
                            var startDt = Ext.Date.clearTime(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name], true),
                                startsOnDate = dt.getTime() == startDt.getTime(),
                                spansFromPrevView = (w == 0 && d == 0 && (dt > rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name]));
                ...
                dt = Ext4.Com.Other.calendar.src.util.Date.add(dt, {days: 1});
            }
        }
    ...
    prepareEventGrid: function (evts, w, d) {
        ...
        evts.each(function (evt) {
            var M = Ext4.Com.Other.calendar.src.data.EventMappings,
                days = Ext4.Com.Other.calendar.src.util.Date.diffDays(
                        Ext4.Com.Other.calendar.src.util.Date.max(me.viewStart, evt.data[M.StartDate.name]),
                        Ext4.Com.Other.calendar.src.util.Date.min(me.viewEnd, evt.data[M.EndDate.name])) + 1;

            if (days > 1 || Ext4.Com.Other.calendar.src.util.Date.diffDays(evt.data[M.StartDate.name], evt.data[M.EndDate.name]) > 1) {
    ...
    onEventDrop: function (rec, dt) {
        if (Ext4.Com.Other.calendar.src.util.Date.compare(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name], dt) === 0) {
            // no changes
            return;
        }
        var diff = dt.getTime() - rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name].getTime();
        rec.set(Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name, dt);
        rec.set(Ext4.Com.Other.calendar.src.data.EventMappings.EndDate.name, Ext4.Com.Other.calendar.src.util.Date.add(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.EndDate.name], {millis: diff}));
    ...
    onCalendarEndDrag: function (start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name] = start;
            o[Ext4.Com.Other.calendar.src.data.EventMappings.EndDate.name] = end;
    ...
    onUpdate: function (ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.EventId.name]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function (ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.EventId.name]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function (ds, rec) {
        ...
        if (this.enableFx && this.enableRemoveFx) {
            this.doRemoveFx(this.getEventEls(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.EventId.name]), {
                remove: true,
                scope: this,
            ...
            this.getEventEls(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.EventId.name]).remove();
            this.refresh();
        }
    },
    ...
    isEventVisible: function (evt) {
        ...
        evEnd = Ext4.Com.Other.calendar.src.util.Date.add(data[M.EndDate.name], {seconds: -1}).getTime();
    ...
    isOverlapping: function (evt1, evt2) {
        var ev1 = evt1.data ? evt1.data : evt1,
            ...
            end1 = Ext4.Com.Other.calendar.src.util.Date.add(ev1[M.EndDate.name], {seconds: -1}).getTime(),
            start2 = ev2[M.StartDate.name].getTime(),
            end2 = Ext4.Com.Other.calendar.src.util.Date.add(ev2[M.EndDate.name], {seconds: -1}).getTime();
    ...
    setViewBounds: function (startDate) {
        var start = startDate || this.startDate,
            ...
            Dt = Ext4.Com.Other.calendar.src.util.Date;
     ...
     * of {@link #Ext4.Com.Other.calendar.src.EventRecord EventRecord} objects
     */
    sortEventRecordsForDay: function (evts) {
        ...
        evts.sortBy(Ext.bind(function (evtA, evtB) {
            ...
            if (this.spansHavePriority) {
                ...
                var diff = Ext4.Com.Other.calendar.src.util.Date.diffDays;
    ...
    moveNext: function (noRefresh) {
        return this.moveTo(Ext4.Com.Other.calendar.src.util.Date.add(this.viewEnd, {days: 1}));
    },
    ...
    movePrev: function (noRefresh) {
        var days = Ext4.Com.Other.calendar.src.util.Date.diffDays(this.viewStart, this.viewEnd) + 1;
    ...
    moveMonths: function (value, noRefresh) {
        return this.moveTo(Ext4.Com.Other.calendar.src.util.Date.add(this.startDate, {months: value}), noRefresh);
    },
    ...
    moveWeeks: function (value, noRefresh) {
        return this.moveTo(Ext4.Com.Other.calendar.src.util.Date.add(this.startDate, {days: value * 7}), noRefresh);
    },
    ...
    moveDays: function (value, noRefresh) {
        return this.moveTo(Ext4.Com.Other.calendar.src.util.Date.add(this.startDate, {days: value}), noRefresh);
    },
    ...
    /**
     * Sets the event store used by the calendar to display {@link Ext4.Com.Other.calendar.src.EventRecord events}.
    ...
    getEventRecord: function (id) {
        var idx = this.store.find(Ext4.Com.Other.calendar.src.data.EventMappings.EventId.name, id);
