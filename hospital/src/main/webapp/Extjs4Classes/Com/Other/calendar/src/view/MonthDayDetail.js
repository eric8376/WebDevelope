 ...
 */
Ext.define('Ext4.Com.Other.calendar.src.view.MonthDayDetail', {
    ...
    requires: [
        ...
        'Ext4.Com.Other.calendar.src.view.AbstractCalendar'
    ],
    ...
    refresh: function () {
        ...
        var eventTpl = this.view.getEventTemplate(),
            ...
            evts = this.store.queryBy(function (rec) {
                    var thisDt = Ext.Date.clearTime(this.date, true).getTime(),
                        recStart = Ext.Date.clearTime(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name], true).getTime(),
                        startsOnDate = (thisDt == recStart),
                        spansDate = false;

                    if (!startsOnDate) {
                        var recEnd = Ext.Date.clearTime(rec.data[Ext4.Com.Other.calendar.src.data.EventMappings.EndDate.name], true).getTime();
        ...
        evts.each(function (evt) {
                ...
                item._renderAsAllDay = item[M.IsAllDay.name] || Ext4.Com.Other.calendar.src.util.Date.diffDays(item[M.StartDate.name], item[M.EndDate.name]) > 0;
                item.spanLeft = Ext4.Com.Other.calendar.src.util.Date.diffDays(item[M.StartDate.name], this.date) > 0;
                item.spanRight = Ext4.Com.Other.calendar.src.util.Date.diffDays(this.date, item[M.EndDate.name]) > 0;
