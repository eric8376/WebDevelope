...
public class ManageOperactionController extends BaseMultiActionController {
	...
	private static String ADD_DICT_SQL = "insert into nursing.t_dict_table(dict_id,group_id,dict_text,group_code,hosp_id,creator_id,creator_dep_id) values(?,?,?,?,?,?,?)";
	private static String DELETE_DICT_SQL = "delete from nursing.t_dict_table where dict_id= ?";
	private static String DELETE_DICT_MAP_SQL = "delete from nursing.t_per_dict_map where parent_id=? and son_id=?";
	private static String ADD_DICT_MAP_SQL = "insert into nursing.t_per_dict_map(parent_id,son_id,map_id) values(?,?,?)" ;
	...
	@RequestMapping(params = "action=addParameter")
	public String addParameter(Model model,String charge_code,String weight) throws Exception {
		
		String paramId = getUUID();
		Result rs=new Result();
		rs.setSuccess(false);
		try {
			int result = jdbcTemplate.update(
	...
	@RequestMapping(params = "action=deleteParameter")
	public String deleteParameter(Model model,String paramId) throws Exception {
		Result rs=new Result();
		rs.setSuccess(false);
		try {
			int result = jdbcTemplate.update(DEL_PARAMETER_SQL,
					new Object[] { paramId });
			rs.setSuccess(result > 0);
		} catch (Exception e) {
			error(e, model, rs);
...
//		String checkExistSql="select count(1) as count from nursing.t_dict_table where hosp_id='"+getHospIdFromSession(request)+"' and group_code='"+groupCode+"' and dict_text='"+dictText+"'";
