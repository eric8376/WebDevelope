...
public class ManageOperactionController extends BaseMultiActionController {
	...
	public ModelAndView updateUser(HttpServletRequest request,
		...
		int result = 0;
		try {
			result = queryDataCmd.getJdbcTemplate().update(
					UPDATE_USER_SQL,
	...
	public ModelAndView deleteUser(HttpServletRequest request,
		...
		int result = 0;
		try {
			result = queryDataCmd.getJdbcTemplate().update(DELETE_USER_SQL,
					new Object[] { userId });
		} catch (DataAccessException e) {
			e.printStackTrace();
		} catch (Exception e) {
	...
	public ModelAndView deleteRecord(HttpServletRequest request,
		...
		int result = 0;
		try {
			result = queryDataCmd.getJdbcTemplate().update(DELETE_RECORD_SQL,
					new Object[] { recordId });
		} catch (DataAccessException e) {
			e.printStackTrace();
		} catch (Exception e) {
	...
	public ModelAndView addDictItem(HttpServletRequest request,
		...
		int result = 0;
		try {
			result = queryDataCmd.getJdbcTemplate().update(ADD_DICT_SQL,
	...
	public ModelAndView deleteDictItem(HttpServletRequest request,
		...
		int result = 0;
		try {
			result = queryDataCmd.getJdbcTemplate().update(DELETE_DICT_SQL,
					new Object[] { dictId });
		} catch (DataAccessException e) {
			e.printStackTrace();
		} catch (Exception e) {
		...
		try {
			...
			for (String sonId : addsonIdList) {
				if (!StringUtils.isEmpty(sonId)) {
					...
					queryDataCmd.getJdbcTemplate().update(ADD_DICT_MAP_SQL,
			...
			for (String sonId : deletesonIdList) {
				if (!StringUtils.isEmpty(sonId)) {
					...
					queryDataCmd.getJdbcTemplate().update(DELETE_DICT_MAP_SQL,
		...
		try {
			...
			for (String ksxmId : addksxmIdList) {
				if (!StringUtils.isEmpty(ksxmId)) {
					...
					queryDataCmd.getJdbcTemplate().update(ADD_ROLE_XM_MAP_SQL,
			...
			for (String ksxmId : deleteksxmIdList) {
				if (!StringUtils.isEmpty(ksxmId)) {
					...
					queryDataCmd.getJdbcTemplate().update(DELETE_ROLE_XM_MAP_SQL,
							new Object[] { ksxmId });
				}
		...
		try {
			...
			for (String roleId : addroleIdList) {
				if (!StringUtils.isEmpty(roleId)) {
					queryDataCmd.getJdbcTemplate().update(ADD_USER_ROLE_MAP_SQL,
			...
			for (String roleId : deleteroleIdList) {
				...
				if (!StringUtils.isEmpty(roleId)) {
					...
					queryDataCmd.getJdbcTemplate().update(DELETE_USER_ROLE_MAP_SQL,
							new Object[] { roleId });
				}
	...
	public ModelAndView changePassword(HttpServletRequest request,
		...
		int result = 0;
		try {
			result = queryDataCmd.getJdbcTemplate().update(UPDATE_PASSWORD_SQL,
