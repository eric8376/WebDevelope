...
Ext.onReady(function () {
    ...
    var deptTree = new Ext.tree.Panel({
        store: new Ext.data.TreeStore({
            ...
            root: root,
            proxy: {
                type: 'ajax', url: 'rolePart.ered?reqCode=departmentTreeInit'
            }
        }),
        title: '',
        applyTo: 'deptTreeDiv',
    ...
    grid.on("cellclick", function (pGrid, td, cellIndex, record, tr, rowIndex, e, eOpts) {
        ...
        if (fieldName == 'roleid' && cellIndex == 1) {
    ...
    var store_part = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            extraParams: {},
            type: 'ajax',
            url: 'rolePart.ered?reqCode=queryParts',
            reader: {
                type: 'json',
                totalProperty: 'TOTALCOUNT', // 记录总数
                root: 'ROOT' // Json中的列表数据根节点
    ...
    menuTreePanel.on('cellclick', function (treeview, htmltext, index, node) {
        ...
        store_part.proxy.extraParams.roleid = roleid;
        store_part.reload();
    });
    ...
    function saveData() {
        ...
        Ext.Ajax.request({
            url: 'rolePart.ered?reqCode=savePartUserGrantDatas',
