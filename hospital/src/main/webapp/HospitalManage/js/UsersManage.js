...
//        clearForm(addUserFormPanel.getForm());
    ...
    function editInit() {
        ...
        Ext.getCmp('password').setValue('@@@@@@');
        Ext.getCmp('password1').setValue('@@@@@@');
        if (!Ext.isEmpty(record.data.empower)) {
            Ext.getCmp('empower').setValue('true');
        }
    ...
    function userGrantInit(userid, deptid, usertype) {
        ...
        var deptTree = new Ext.tree.Panel({
            store: new Ext.data.TreeStore({
                ...
                proxy: {
                    ...
                    type: 'ajax', url: './HospitalManageAction.ered?reqCode=userGrantInit'
                }
            }),
            title: '',
            rootVisible: false,
        ...
        var userGrantTabPanel = new Ext.panel.Panel({
            activeTab: 0,
            width: 600,
            height: 250,
            border: false,
            ...
            buttons: [
                {
                    ...
                    handler: function () {
                        ...
                        var roleid = '';
                        if (!Ext.isEmpty(node)) {
                            roleid = node[0].data.id
                        }
                        Ext.Ajax.request({
                            url: './HospitalManageAction.ered?reqCode=saveUserGrant',
                            success: function (response) {
