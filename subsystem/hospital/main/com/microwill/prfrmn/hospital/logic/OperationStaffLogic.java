...
public class OperationStaffLogic extends UserTypeLogic{
	...
	public String getXMList() throws Exception {
		String sql="select t.dict_id,t.dict_text from hospital.t_dict_table t where hosp_id=? and t.dict_id in (select xm_id from t_per_record t1 where t1.user_name=? and t1.ks_id=?  group by t1.xm_id)";
		String ks_id=(String)getLoginedUserContext().get("ks");
		String real_name=(String)getLoginedUserContext().get("real_name");
		List list=getQueryDataCmd().getJdbcTemplate().queryForList(sql,new Object[]{getLoginedUserContext().get("hosp_id"),real_name,ks_id});
		String jsonTxt=JSONExecuteHelp.parseJSONText(list);
		return jsonTxt;
	}
