...
public class XmlHelper {
    ...
    public static final Dto parseXml2DtoBasedNode(String pStrXml) {
        ...
        for (Iterator it = elNode.elementIterator(); it.hasNext(); ) {
            ...
            if (G4Utils.isNotEmpty(leaf.attributeValue("type"))) {
                value = RecursiveParsingToCloneable(leaf);
            } else {
                value = leaf.getData();
            }
            outDto.put(leaf.getName(), value);
        }
        return outDto;
    }

    private static Object RecursiveParsingToList(Element leaf) {
        List tlist = new ArrayList();
        for (Iterator it = leaf.elementIterator(); it.hasNext(); ) {
            Element el = (Element) it.next();
            if (G4Utils.isNotEmpty(el.attributeValue("type"))) {
                tlist.add(RecursiveParsingToCloneable(el));
            } else {
                tlist.add(el.getData());
            }
        }
        return tlist;
    }

    private static Object RecursiveParsingToMap(Element leaf) {
        Dto dto = new BaseDto();
        List<Element> list = leaf.elements();
        int count = list.size();
        Object obj = null;
        for (int i = 0; i < count; i++) {
            Element el = list.get(i);
            if (G4Utils.isNotEmpty(el.attributeValue("type"))) {
                obj = RecursiveParsingToCloneable(el);
            } else {
                obj = el.getData();
            }
            dto.put(el.getName(), obj);
        }
        return dto;
    }

    private static Object RecursiveParsingToCloneable(Element leaf) {
        String type = leaf.attributeValue("type");
        Object obj = null;
        if ("list".equals(type)) {
            obj = RecursiveParsingToList(leaf);
        } else if ("map".equals(type)) {
            obj = RecursiveParsingToMap(leaf);
        }
        return obj;
    }
    ...
    public static final String parseDto2Xml(Dto pDto, String pRootNodeName) {
        ...
        while (keyIterator.hasNext()) {
            ...
            String value = null;
            Object obj = pDto.get(key);
            if (obj instanceof Cloneable) {
                recursiveElementToCloneable(leaf, obj);
            } else {
                value = G4Utils.isNotEmpty(obj) ? obj.toString() : "";
                leaf.setText(value);
            }
        }
        // 将XML的头声明信息截去
     ...
     * @return
     */
    public static final String parseSpecialField2Xml(Dto dto, String[] fields, String pRootNodeName) {
    ...
    public static final String parseDto2Xml(Dto pDto, String pRootNodeName, String pFirstNodeName) {
        ...
        recursiveElementToMap(firstEl, pDto);
        // 将XML的头声明信息丢去
        String outXml = document.asXML().substring(39);
        return outXml;
    }

    private static void recursiveElementToMap(Element firstEl, Map map) {
        Iterator keyIterator = map.keySet().iterator();
        while (keyIterator.hasNext()) {
            String key = (String) keyIterator.next();
            //String value = pDto.getAsString(key);
            Element recursiveEl = firstEl.addElement(key);
            Object obj = map.get(key);
            String value = null;
            if (obj instanceof Cloneable) {
                recursiveElementToCloneable(recursiveEl, obj);
            } else {
                value = obj.toString();
                recursiveEl.setText(value);
            }
        }
    }

    private static void recursiveElementToList(Element firstEl, List list) {
        int count = list.size();
        for (int i = 0; i < count; i++) {
            Element recursiveEl = firstEl.addElement("item");
            String value = null;
            if (list.get(i) instanceof Cloneable) {
                recursiveElementToCloneable(recursiveEl, list.get(i));
            } else {
    ...
    private static void recursiveElementToCloneable(Element firstEl, Object obj) {
        if (obj instanceof List) {
            firstEl.addAttribute("type", "list");
            recursiveElementToList(firstEl, (List) obj);
        } else if (obj instanceof Map) {
            firstEl.addAttribute("type", "map");
            recursiveElementToMap(firstEl, (Map) obj);
        }
    }
