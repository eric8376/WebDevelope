...
<ol id="grid">
	<% 
		List list =(ArrayList)request.getAttribute("logoList");
		for(int i=0;i<list.size();i++){
		Map m=(Map)list.get(i);		
		System.out.println(m.get("ALARM_ID"));
	 %>
	 <li><a id="icons" href="<%=request.getContextPath()%>/main/touch/resAlarm.jsp?alarmId=<%=m.get("ALARM_ID")%>&title=<%=m.get("ALARM_NAME") %>" target="_blank">
        <img src="<%=request.getContextPath()%>/style/touch/images/unicom/main/icon_big/transalarm.png" alt="Icons" height="72" width="72" />
        <span><%=m.get("ALARM_NAME") %></span>
    </a></li>   
    <%}%>
</ol>
</body>
