...
this.deleteData = function (){
  	 ...
  	 while(this.getRowId(0)){
	 	 this.deleteRow(this.getRowId(0));
	 }
}
this.loadData = function(dataArr){
     this.deleteData();
	 this.gridDataArray= new Array(0);
	 if(typeof(dataArr)=="undefined"||dataArr==null || dataArr.length<=0 ||!dataArr[0])return;
	 this.gridDataArray=dataArr;
	 for(var i=0;i<dataArr.length;i++){
	 	var dataItem = dataArr[i];
	 	var colTexts =new Array();
	 	for(var j=0;j<this.hdr.rows[1].cells.length;j++){
	 		colTexts.push(dataItem[this.hdr.rows[1].cells[j].id]||"");
	 	}
	 	this.addRow(i+1,colTexts.join(","),i);
	}
	this.setSizes();
 ...
 this.setHeaderCol(i,this.hdrLabels[i],this.cellId[i],this.cellShow[i]);
}
...
this.doColResize_2 = function(){
 //this._setColumnSizeR(0,"200");
 //this.doOnScroll(0,1);
 //this.setSizes();
 ...
 if(ind<r.childNodes.length)
{
 ...
 this.setHeaderCol = function(col,label,colId,displayValue){
 var z=this.hdr.rows[1];
 ...
 hdrHTML+=label;
 hdrHTML+="</div>";
 z.cells[col].id=colId.toString();
 z.cells[col].style.display=displayValue;
 ...
 var arShow = new Array(0);
 var arId = new Array(0);
...
this.setColShow = function(alStr){
this.cellShow =alStr.split(this.delim);	
}
this.setColId = function(alStr){
 this.cellId = alStr.split(this.delim);	
} 
this.setDblClick = function(alStr){
  
 this.dblClickFunction = alStr;	
 ...
 if((!_isOpera)||(_OperaRv>8.5))
{
 ...
 //this.obj.ondblclick = new Function("e","if(!this.grid.wasDblClicked(e||window.event)){return false};if(this.grid._dclE)this.grid.editCell(e||window.event);(e||event).cancelBubble=true;");
  //this.obj.ondblclick = this._dbClick;
 ...
 dhtmlxEvent(r,"dblclick",function(){
	if(typeof(r.grid.dblClickFunction)=="undefined"||r.grid.dblClickFunction==null) return;
	  eval(r.grid.dblClickFunction(r.grid.gridDataArray[r.idd-1]));
	});
 for(var i=0;i<this.hdr.rows[0].cells.length;i++){
 
 var c = document.createElement("TD");

 var cId =this.hdr.rows[1].cells[i].id;
 if(cId==null ||cId.length<=0) cId=r.idd;
 if(this._enbCid)c.id="c_"+cId+"_"+i;
 
 c._cellIndex = i;
 if(this.dragAndDropOff)this.dragger.addDraggableItem(c,this);
 c.align = this.cellAlign[i];
 c.style.display = this.cellShow[i];
