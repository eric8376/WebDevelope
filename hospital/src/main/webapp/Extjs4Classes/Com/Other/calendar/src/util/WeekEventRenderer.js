 ...
 */
Ext.define('Ext4.Com.Other.calendar.src.util.WeekEventRenderer', {

    requires: ['Ext4.Com.Other.calendar.src.util.Date'],
        ...
        render: function (o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = Ext4.Com.Other.calendar.src.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = Ext4.Com.Other.calendar.src.util.Date.add(startOfWeek, {
                            days: dayCount,
                            millis: -1
                        });
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[Ext4.Com.Other.calendar.src.data.EventMappings.IsAllDay.name] || evt.isSpanStart;
                                    item.spanLeft = item[Ext4.Com.Other.calendar.src.data.EventMappings.StartDate.name].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[Ext4.Com.Other.calendar.src.data.EventMappings.EndDate.name].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = Ext4.Com.Other.calendar.src.util.Date.diffDays(dt, item[Ext4.Com.Other.calendar.src.data.EventMappings.EndDate.name]) + 1,
