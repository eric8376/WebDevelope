...
import org.nxstudio.util.base.TypeCaseHelper;
...
public class G4Utils {
    ...
    public static String getWeekDayByDate(String strdate) {
        ...
        return dayNames[getWeekDayByDateToInt(strdate)];
    }
     ...
     * @return
     */
    public static int getWeekDayByDateToInt(String strdate) {
     ...
     * @param cls         用来定位的类
     * @return 相对路径所对应的绝对路径
     * @throws java.io.IOException 因为本方法将查询文件系统，所以可能抛出IO异常
     */
     ...
     * @return
     */
    public static String minuteToWorkingDay(long minute) {
        String retu = "";

        long min = minute % 60;
        long hour = minute / 60 % 8;
        long day = minute / 60 / 8;
     ...
     * @return
     */
    public static String DateToFormat(Date time, String format) {
     ...
     * @return
     */
    public static Date StringToDate(String srcString, String format) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat(format);
        return sdf.parse(srcString);
    }
     ...
     * 验证MD5的值。
     *
     * @param original 原始值，要进行MD5计算的
     * @param md5      以计算的md5的值。
     * @return
     */
    public static boolean verifyMd5(String original, String md5) throws NoSuchAlgorithmException {
        String correct = encode(original);
        if (StringUtils.isNotBlank(correct) && correct.equals(md5)) {
            return true;
        }
        return false;
    }
    ...
    public static String encode(String source) throws NoSuchAlgorithmException {
        ...
        byte[] tmp = md.digest(source.getBytes());
    ...
    public static List getFieldArrayByList(List<Dto> list, String field) {
     ...
     * @return
     */
    public static Dto getCallerInfo(int i) {
        StackTraceElement stack[] = (new Throwable()).getStackTrace();
        StackTraceElement s = stack[i];
        Dto dto = new BaseDto();
        dto.put("entityclass", s.getClassName());
        dto.put("promethod", s.getMethodName());
        return dto;
    }
     ...
     * @return
     */
    public static String[] stringsArrayToHeavy(String[] valueString) {
        TreeSet<String> tr = new TreeSet<String>();
        for (int i = 0; i < valueString.length; i++) {
            tr.add(valueString[i]);
        }
        String[] newString = new String[tr.size()];
        for (int i = 0; i < newString.length; i++) {
            newString[i] = tr.pollFirst();//从TreeSet中取出元素重新赋给数组
        }
        return newString;
    }
     ...
     * @param fieldname
     */
    public static void replaceAllTInDate(Dto dto, String fieldname) {
        dto.put(fieldname, dto.getAsString(fieldname).replaceAll("T", " "));
    }
